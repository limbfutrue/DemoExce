...
public class LineChartManager {
     ...
     * @param color
     */
    public LineChartManager setVLine(int color){
     ...
     * @param color
     */
    public LineChartManager setHLine(int color){
        leftAxis.setGridColor(color);
        return this;
    }

    public LineChartManager setXTextColor(int color){
        xAxis.setTextColor(Color.WHITE);
        return this;
    }
    public LineChartManager setYTextColor(int color){
    ...
    private void initLineDataSet(LineDataSet lineDataSet, int color, boolean mode) {
        lineDataSet.setColor(color);//折线 线条颜色
        lineDataSet.setCircleColor(color);// x y 轴交叉点 颜色
        lineDataSet.setLineWidth(1f);//折线 宽
        lineDataSet.setCircleRadius(3f);// x y 轴交叉点 半径
        //设置曲线值的圆点是实心还是空心
     ...
     * @param labelCount
     */
    public void setYAxis(float max, float min, int labelCount, boolean showRight) {
        if (max < min) {
            return;
        }
...
//        libaoming   修改，是否显示右侧
        rightAxis.setAxisMaximum(showRight ? max : 0);
        rightAxis.setAxisMinimum(showRight ? min : 0);
